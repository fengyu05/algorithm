...
/*  
 求 val=min( (|xi-x|+|yi-y|+|zi-z|)/pi )
 
 二分val
 判断是否满足|xi-x|+|yi-y|+|zi-z|<=pi*val i=[1..n]
 
 即判断下列不等式是否有解
 a*x+b*y+c*z<=min(pi*val+a*xi+b*yi+c*zi) i=[1..n]
 st. a,b,c=[1,-1]
 
 令 best[a][b][c]=min(pi*val+a*xi+b*yi+c*zi) i=[1..n]
 即判断是否满足：0=a*x+b*y+c*z + -a*x-b*x-c*x <=best[a][b][c]+best[-a][-b][-c]
*/
int n;
