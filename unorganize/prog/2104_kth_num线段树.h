...
void TNode::construct(int l,int r,int dep)
{
    ...
    while(i<=mid && j<=r){
        ...
        else SortArray[dep][k++]=SortArray[dep+1][j++];
    }
...
int main ()
{
    ...
    /*
    for(i=0;i<5;i++)
        for(j=0;j<8;j++) SortArray[i][j]=0;
    */
    Node[0].construct(0,N-1,0);
    /*
    for(i=0;i<5;i++){
        for(j=0;j<8;j++) printf("%d ",SortArray[i][j]);
        printf("\n");
    }
    */
    for(i=0;i<Q;i++){
        ...
        while(low+1<high){
            mid=(low+high)>>1;
            Key=SortArray[0][mid];
            int Rank=Node[0].GetRank();  //当前关键字在选定区间内的排位 
     //       printf("Rank=%d  key=%d  Index=%d\n",Rank,Key,Index);
            if(Rank>=Index)high=mid;
            else low=mid;
        }
        printf("%d\n",SortArray[0][low]);
    }
